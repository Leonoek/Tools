...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误: 这不是一个正确的文件. 它有 LF 行结束问题.
echo:
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "输入菜单项 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:
call :_color2 %_White% "              [R] " %_Green% "帮助"
echo:              [0] 返回
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "输入菜单项:"
...
call :_color %Green% "$OEM$ 文件夹在桌面创建成功."
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回到主菜单, 选择 故障排除, 并运行修复许可选项."
) else (
set "_exitmsg=退出"
set "_fixmsg=在 MAS 文件夹, 运行故障排除脚本并选择修复许可选项."
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo HWID 激活仅支持 for Windows 10/11.
echo 使用 在线 KMS 激活选项.
goto dk_done
)
...
call :dk_color %Red% "启动 [sppsvc] 服务失败, 其余过程可能需要很长时间..."
echo:
)
...
call :dk_color %Gray% "检查 Windows Update 阻止程序        [发现]"
if defined applist echo: %serv_e% | find /i "wuauserv" %nul% && (
call :dk_color %Blue% "Windows 更新无法工作. 如果已禁用请启用."
...
call :dk_color %Gray% "检查 应用商店 阻止程序           [发现]"
)
...
call :dk_color %Red% "检查 ClipUp.exe 文件                [未找到, 中止进程]"
call :dk_color2 %Blue% "查看帮助页" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查 Ticket 迁移               [失败]"
)
...
call :dk_color2 %Blue% "查看帮助页" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
if %osSKU%==175 call :dk_color %Red% "%winos% 不支持在非云平台激活."
...
if defined serv_cor (
set error=1
call :dk_color %Red% "检查 Corrupt Services               [%serv_cor%]"
)
...
call :dk_color %Blue% "重启系统以修复禁用服务错误 1058."
set showfix=1
)
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查引导模式                      " %Blue% "[系统在安全模式下运行. 请以正常模式运行.]"
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查审核模式                     " %Blue% "[IMAGE_STATE_COMPLETE 状态未发现, 请以正常模式运行.]"
)
...
set error=1
call :dk_color %Red% "检查 Powershell                     [未响应]"
)
...
set error=1
call :dk_color %Red% "检查 评估包                  [非评估许可证安装在评估 Windows 中]"
if /i %dism_error%==[0x800F0805] (
for %%# in (4 125 126 188 191 205) do if "%osSKU%"=="%%#" (
call :dk_color %Blue% "评估 Windows 无法激活, 不同的许可证安装可能导致错误."
call :dk_color %Blue% "推荐安装完整版的 %winos%."
call :dk_color %Blue% "你可以从此下载: %mas%genuine-installation-media.html"
set showfix=1
)
)
)
...
set error=1
call :dk_color %Red% "检查 WMI                            [未响应]"
call :dk_color %Blue% "在 MAS 中, 转到 故障排除 并运行修复 WMI 选项."
set showfix=1
)
...
call :dk_color %Red% "检查 SLC/WMI/REG SKU                [发现差异 - SLC:%slcSKU% WMI:%wmiSKU% Reg:%regSKU%]"
)
...
call :dk_color %Red% "检查 SPP 注册表键值               [发现不正常的模块号]"
call :dk_color %Blue% "可能由游戏欺骗引起. 帮助: %mas%troubleshoot"
set error=1
set showfix=1
)
...
if defined wpaerror call :dk_color %Red% "检查 WPA 注册表键值              [找到错误] [注册表数量 - !count!]"
)
...
call :dk_color %_Yellow% "按 任意键 %_exitmsg%..."
pause %nul1%
exit /b
...
::  7th column = Key Type
::  8th column = WMI Edition ID (仅作参考)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回到主菜单, 选择 故障排除 并修复许可选项."
) else (
set "_exitmsg=退出"
set "_fixmsg=在 MAS 文件夹, 运行 故障排除 并选择修复许可选项."
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo Ohook 激活支持 Windows 8 及更高版本和对应服务器版本.
goto dk_done
)
...
echo:
echo 错误代码: %errorlevel%
call :dk_color %Red% "启动 [sppsvc] 服务失败, 剩下的过程可能需要很长时间..."
echo:
)
...
echo:
echo 如果仅安装了应用商店版 Office, 需要安装完整版.
)
echo:
...
call :dk_color %Gray% "检查多个 Office 安装        [已发现, 建议只安装一个版本.]"
)
...
if defined sub_next echo 移除 Office vNext 阻止程序             [成功]
...
if %upk_result%==1 echo 正在卸载 其他/宽限 密钥           [成功]
if %upk_result%==2 call :dk_color %Red% "正在卸载 其他/宽限 密钥           [失败]"
...
call :dk_color %Blue% "关闭正在运行的 Office 应用程序后重试."
) else (
...
call :dk_color %Gray% "安装缺失的许可文件        [Office %oVer%.0 %_prod%] [成功]"
) || (
set error=1
call :dk_color %Red% "安装缺失的许可文件        [Office %oVer%.0 %_prod%] [失败]"
)
...
echo 正在提取自定义 %_hook% 为         ["%_hookPath%\sppc.dll"] [成功]
) else (
set error=1
call :dk_color %Red% "创建系统符号链接 sppc.dll             [失败]"
call :dk_color %Red% "正在提取自定义 %_hook%            [失败]"
echo ["%_hookPath%\sppc.dll"]
echo:
call :dk_color %Blue% "关闭包括 Outlook 在内的所有 Office 应用后重试."
call :dk_color %Blue% "如果仍未解决, 请重启系统后重试."
)
...
set error=1
call :dk_color %Red% "在脚本中检查产品              [Office %oVer%.0 %%# 未找到]"
echo 确认你使用最新版的 MAS 脚本.
)
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回主菜单, 选择 故障排除 并运行修复许可选项."
) else (
set "_exitmsg=退出"
set "_fixmsg=在 MAS 文件夹中, 运行 故障排除 并选择修复许可选项."
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo KMS38 激活支持 Windows 10/11/Server, 14393 及更高.
goto dk_done
)
...
echo:
echo 检查附加菜单中的 更改 Windows 版本 选项.
) else (
echo 评估版本无法被激活.
echo 需要安装完整版本的 %winos%
echo:
echo 从此下载:
echo %mas%genuine-installation-media.html
)
...
call echo 正在检查已安装的产品密钥          [部分密钥 - %%_partial%%] [Volume:GVLK]
)
...
call :dk_color %Red% "复制 clipup.exe 文件到              [%systemroot%\System32\] [失败] Aborting..."
goto :k_final
)
)
...
call :dk_color %Red% "正在应用 SKU-ID 重组                   [失败]"
)
call :dk_refresh
...
call :dk_color %Red% "%winos% 不支持在非云平台上激活."
)
...
for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 系统中无法找到 wmic.exe 文件.
if %winbuild% GEQ 22621 echo 确保 WMIC 在可选功能中已启用.
goto Done
)
...
echo WMI 在系统中无响应.
echo:
echo 在 MAS 中, 转到 故障排除 并运行修复 WMI 选项.
goto Done
)
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "输入菜单项 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
set "_mOuwp=检测到 Office 365/2016 UWP 版不被支持"
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo 检查 KMS 激活 ID 失败. &call :CheckWS
    exit /b
    )
  )
)
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 安装密钥: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 安装密钥: %%x
set ESU_ADD=0
call :keys %app%
if "%_key%"=="" (echo 找不到关联的 KMS 客户端密钥&exit /b)
...
if %OSType% EQU Win7 echo 由于 OEM BIOS 不合格, 此计算机上的 Windows 7 无法被 KMS 激活.
echo 请参阅 帮助 了解详细信息.
exit /b
)
...
if %_Office15% EQU 0 (echo 错误: Office C2R 许可集成器未检测到&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if not [%Act_OK%]==[1] (
echo.
echo 如有任何问题, 查看 https://mass%-%grave.dev/troubleshoot
)
...
call :_color %Red% "你的互联网连接中 1688 端口被阻止."
echo.
echo 原因:   可能连接到受限制 的互联网 [Office/College],
echo           或者被防火墙阻止.
echo.
echo 使用其他互联网连接或使用脱机 KMS 激活
echo           https://github.com/abbodi1406/KMS_VL_ALL_AIO
) else (
echo  1688 端口连接测试通过.
echo.
echo 确保系统文件没有被防火墙阻止.
echo 如果问题仍然存在, 请尝试脱机 KMS 激活
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
)
echo.
)

echo 在此情况下, KMS 服务器不存在问题.
exit /b
...
echo __________________________________________________________________________________________
echo.
call :_color %Green% "在线 KMS 已成功完全卸载."
echo __________________________________________________________________________________________
)
...
echo %_dest%\Info.txt
echo.
(if defined ActTask (echo Scheduled Tasks created:) else (echo 创建计划任务:))
echo \激活-续期 [每周]
if defined ActTask (echo \激活-仅一次)
echo __________________________________________________________________________________________
echo.
echo 说明:
echo 如果发现互联网连接, 激活将每周更新.
echo 只会更新已安装的 KMS 许可证, 不会将任何许可证转换为 KMS.
echo __________________________________________________________________________________________
echo.
if defined ActTask (
...
echo __________________个_______________________________________________________________________
)
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Version>1.0</Version>
    <Description>在线 KMS 激活续期 - 每周任务</Description>
...
if %_wmic% EQU 1 for /f "tokens=2 delims==" %%x in ('"wmic path !_path! where ID='!_actid!' get Name /VALUE" 2^>nul') do call echo 激活: %%x
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%x in ('%psc% "(([WMISEARCHER]'SELECT Name FROM !_path! WHERE ID=''!_actid!''').Get()).Name | %% {echo ('Name='+$_)}" 2^>nul') do call echo 激活: %%x
exit /b
...
echo 产品激活失败: 0xC004F035
if [%1]==[act_win] if %winbuild% LSS 9200 echo 由于 OEM BIOS 不合格, 此计算机上的 Windows 7 无法被 KMS 激活.
exit /b
)

if %errorcode% EQU -1073417728 (
echo 产品激活失败: 0xC004F200
echo Windows 需要重建与激活相关文件.
set _tserror=1
exit /b
)
...
echo %line2%
echo.
powershell "write-host -back 'Black' -fore 'Yellow' 'Office 已使用 Ohook 激活方法永久激活.'; write-host -back 'Black' -fore 'Yellow' '你可以忽略下面 Office 激活状态.'"
echo.
)
...
echo %line2%
echo.
powershell "write-host -back 'Black' -fore 'Yellow' 'Office 已使用 Ohook 激活方法永久激活.'; write-host -back 'Black' -fore 'Yellow' '你可忽略下面 Office 激活状态.'"
echo.
)
...
echo %Description%| findstr /i TIMEBASED_ 1>nul && (set cTblClient=1&set _mTag=基于时间)
echo %Description%| findstr /i VIRTUAL_MACHINE_ACTIVATION 1>nul && (set cAvmClient=1&set _mTag=自动虚拟机)
cmd /c exit /b %LicenseStatusReason%
set "LicenseReason=%=ExitCode%"
set "LicenseMsg=激活期限: %GracePeriodRemaining% 分钟 (%_gpr% 天)"
...
%eline%
echo 检测到不支持的操作系统版本.
echo 此命令仅支持 Windows 8/8.1/10/11 及对应的服务器版本.
goto :at_back
)
...
call :_color2 %_White% "     - " %Gray% "确保 Windows Update 正常工作."
echo:
echo %line%
echo:
...
echo:
echo      如果 SFC 无法修复某个问题, 请再次运行该命令看是否可在下一次运行.
echo      有时可能需要运行 sfc/scannow 命令三次并且每次重新启动电脑,
echo      才能完全修复所有可能的问题.
echo:   
echo %line%
echo:
...
echo %line%
echo:
call :_color %Blue% "重建 ClipSVC 许可"
echo:

if %winbuild% LSS 10240 (
echo ClipSVC 许可重建仅在 Win 10/11 和相应服务器支持.
echo 正在跳过...
goto :cleanvnext
)
...
echo %line%
echo:
call :_color %Blue% "正在清除 Office vNext 许可"
echo:
...
echo 未发现 - !ProgramData!\Microsoft\Office\Licenses\
)
...
echo 未发现 - !_Local!\Microsoft\Office\Licenses\
)
...
if not defined regfound echo 未发现 - Office vNext 注册表项
...
echo:
echo 正在检查 WPA 注册表项...
call :_color %Red% "[找到错误] [注册表Registry 数量 - !count!]"
)
)
...
if not defined token (
call :_color %Red% "tokens.dat 文件未发现."
) else (
echo tokens.dat 文件: [%token%]
)
...
echo:
call :_color %_Yellow% "在弹出一个窗口中, 选择[快速]修复选项..."
...
call :_color %Red% "[失败]"
echo:
call :_color %Blue% "建议选择 [重启] 选项并再次应用修复 WMI 选项."
echo %line%
echo:
...
echo:
echo 正在注册 .dll 文件并编译 .mof 和 .mfl 文件
...
if %~z0 GEQ 200000 (set "_exitmsg=返回") else (set "_exitmsg=Exit")
...
if not defined applist (
%eline%
echo Activation IDs not found. 正在中止...
echo:
echo 查看帮助页. %mas%troubleshoot
goto ced_done
)
)
...
%line%
echo:
if defined dismnotworking call :dk_color %Red% "DISM.exe 未响应."
...
%line%
echo:
call :dk_color %Gray% "你可以将此版本 [%osedition%] [%winbuild%] 更改为以下版本之一."
if defined dismnotworking (
call :dk_color %_Yellow% "说明 - DISM.exe 未响应."
if /i "%osedition:~0,4%"=="Core" call :dk_color %_Yellow% "     - 一旦更改为专业版, 你将看到更多可供选择的版本选项."
)
%line%
echo:
...
echo [0]  %_exitmsg%
echo:
call :dk_color %_Green% "输入菜单项并按回车键:"
set /p inpt=
...
%line%
echo:
if defined dismnotworking call :dk_color %_Yellow% "DISM.exe 无响应."
echo 更改当前版本 [%osedition%] %winbuild% 为 [%targetedition%]
echo:

if %_dismapi%==1 (
call :dk_color %Green% "说明-"
echo:
echo  - 继续前保存你的工作, 系统将自动重启.
echo:
echo  - 一旦版本更改, 你将需使用 HWID 选项激活.
%line%
echo:
...
echo:
if defined dismnotworking call :dk_color %_Yellow% "说明 - DISM.exe 无响应."
echo 更改当前版本 [%osedition%] %winbuild% 为 [%targetedition%]
echo:
call :dk_color %Blue% "重要信息 - 继续前请保存你的工作, 系统将自动重启."
echo:
...
call :dk_color %Blue% "你必须在此阶段重启系统."
